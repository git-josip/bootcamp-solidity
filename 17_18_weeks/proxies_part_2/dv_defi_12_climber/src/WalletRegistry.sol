...
contract WalletRegistry is IProxyCreationCallback, Ownable {
    ...
    function proxyCreated(
        ...
        require(bytes4(initializer[:4]) == Safe.setup.selector, "Wrong initialization");

        // Ensure wallet initialization is the expected
        require(Safe(walletAddress).getThreshold() == MAX_THRESHOLD, "Invalid threshold");
        require(Safe(walletAddress).getOwners().length == MAX_OWNERS, "Invalid number of owners");       
